...
def get_api_key(args_key=None):
    """获取API密钥，优先使用命令行参数，其次是 CFG 配置，最后是环境变量"""
    if args_key:
        return args_key
    ...
    # 兼容老的环境变量
    api_key = os.environ.get("GOOGLE_API_KEY") or os.environ.get("APIYI_API_KEY")
    if not api_key:
        print("错误: 未检测到有效的 API Key")
        print("请在 project_root/config/config.py 中设置 CFG.api_key")
        print("或者设置 GOOGLE_API_KEY 环境变量")
        print("也可以使用 --api-key 参数临时指定")
        sys.exit(1)
...
【环境变量】
  export GOOGLE_API_KEY="your-api-key"
    ...
    default_output_dir = project_root / "generated_image"
    default_output_dir.mkdir(parents=True, exist_ok=True)

    # 如果没有指定文件名，自动生成在默认目录
    if args.filename is None:
        args.filename = str(default_output_dir / generate_filename(args.prompt))
    else:
        # 如果指定了路径，确保父目录存在
        out_path = Path(args.filename)
        if not out_path.is_absolute():
             args.filename = str(default_output_dir / args.filename)
