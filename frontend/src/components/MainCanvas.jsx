...
export default function MainCanvas() {
    const { genUI, setShowChatbox } = useUI();
    const hasGenUI = genUI && genUI.reactCode && genUI.reactCode !== 'none';
    ...
    const apps = [
        ...
        { id: 'data', name: 'Knowledge', icon: <Database size={36} />, color: 'from-cyan-400 to-cyan-600' },
        { id: 'activity', name: 'Sensors', icon: <Activity size={36} />, color: 'from-orange-400 to-orange-600' },
        { id: 'security', name: 'Guard', icon: <Shield size={36} />, color: 'from-red-400 to-red-600' },
    ...
    return (
        <div className="relative w-full h-full flex flex-col justify-center items-center pointer-events-none">
            ...
            {/* 2. Active GenUI Layer */}
            {hasGenUI && (
                <div className="absolute inset-0 z-10 p-4 sm:p-8 flex justify-center items-center pointer-events-none">
                    <GenUI />
                </div>
            )}
